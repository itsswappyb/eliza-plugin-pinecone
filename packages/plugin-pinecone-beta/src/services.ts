...
//         const randomRover =
//             roverNames[Math.floor(Math.random() * roverNames.length)];
//         const selectedRover = rovers[randomRover as keyof typeof rovers];

//         // Get random camera for selected rover
//         const randomCamera =
//             selectedRover.cameras[
//                 Math.floor(Math.random() * selectedRover.cameras.length)
//             ];
...
export const createChatService = (apiKey: string, agentName: string) => {
    const chat = async (request: ChatRequest): Promise<ChatResponse> => {
        const response = await fetch(
            `https://prod-1-data.ke.pinecone.io/assistant/chat/${agentName}`,
            {
                method: "POST",
                headers: {
                    "Api-Key": apiKey,
